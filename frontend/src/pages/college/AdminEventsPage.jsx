...
export function AdminEventsPage({ adminUser, onLogout }) {
  useSEO({ title: "Manage Events", description: "Create, edit, and manage NSS events. Track attendance and issue certificates.", keywords: "manage events, admin events", noIndex: true });
    ...
    const confirmDelete = async () => {
        ...
        try {
            await adminApi.delete(`/admin/events/${eventToDelete.id}/`);
    ...
    const markPresent = async (registrationId) => {
        ...
        try {
            ...
            toast.success("Attendance marked. Certificate generated automatically.");
    ...
    return (
        <main className="min-h-screen pb-24 bg-white">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-10 rounded-[1.5rem] shadow-[0_20px_60px_rgba(0,0,0,0.06)] border border-slate-100">
                    ...
                    <form onSubmit={submitEvent} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <input className={`${inputClasses} md:col-span-2`} placeholder="Event Title (e.g. Clean Purnea Drive)" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} disabled={submitLoading} required />
                            ...
                            <input type="number" step="0.25" min="0.25" className={inputClasses} placeholder="Hours per Volunteer" value={form.hours_per_volunteer} onChange={(e) => setForm({ ...form, hours_per_volunteer: e.target.value })} disabled={submitLoading} />
                        <input type="date" min={today} className={inputClasses} value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} disabled={submitLoading} required />
                        <input className={inputClasses} placeholder="Location / Meeting Point" value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} disabled={submitLoading} required />
                        <textarea className={`${inputClasses} md:col-span-2 resize-none h-32`} placeholder="Mission Description" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} disabled={submitLoading} />
                        <input className={`${inputClasses} md:col-span-2`} placeholder="Program Coordinator Name" value={form.program_coordinator_name} onChange={(e) => setForm({ ...form, program_coordinator_name: e.target.value })} disabled={submitLoading} />
                            <div className="md:col-span-2 flex gap-3">
                                <button disabled={submitLoading} className="flex-1 py-5 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:bg-emerald-600 transition-all shadow-xl shadow-slate-200 flex items-center justify-center gap-2 disabled:opacity-50">
                                    {submitLoading ? (
                                        <>
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4 border-b border-slate-100 text-right">
                                                <div className="inline-flex gap-2">
                                                    ...
                                                    <button onClick={() => triggerDelete(e)} className="w-10 h-10 bg-red-50 text-red-500 rounded-xl flex items-center justify-center hover:bg-red-600 hover:text-white transition-all"><FiTrash2 /></button>
                                                </div>
                                            </td>
                                        </motion.tr>
                                    ))}
            ...
            <ConfirmDialog 
                ...
                title="Delete Event?"
                message={`This will permanently delete "${eventToDelete?.title || 'this event'}" and all associated registrations.`}
