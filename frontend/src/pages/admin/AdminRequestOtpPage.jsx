...
export function AdminRequestOtpPage() {
    ...
    const submit = async (e) => {
        ...
        try {
            const res = await adminApi.post("/admin/auth/request-otp/", { email });
            setMessage({ type: "success", text: res.data.message || "OTP sent to your email." });
            if (res.data.otp_reset_token) {
                sessionStorage.setItem("admin_otp_reset_token", res.data.otp_reset_token);
                sessionStorage.setItem("admin_otp_reset_email", email);
            }
            ...
            setMessage({
                type: "error",
                text: err.response?.data?.detail || "OTP request failed. Please check your email."
            });
        } finally {
            setLoading(false);
        }
        ...
        <main className="min-h-screen pb-24 bg-white">
            ...
            <section className="max-w-xl mx-auto px-6 -mt-32 relative z-20">
                ...
                >
                    <div className="bg-slate-900 px-10 py-5 flex justify-between items-center">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic flex items-center gap-2">
                            <FiSmartphone className="text-slate-500" /> Email OTP Request
                        </span>
                    </div>
                    ...
                    <form onSubmit={submit} className="p-10 md:p-14 space-y-6">
                        ...
                        <div className="pt-4 space-y-6">
                            ...
                            >
                                {loading ? <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : <>Send OTP <FiArrowRight className="group-hover:translate-x-1 transition-transform" /></>}
                            </button>
                ...
                <p className="text-center mt-10 text-slate-400 text-[10px] font-black uppercase tracking-widest">
                    Remembered your password? <Link to="/admin/login" className="text-slate-900 underline">Back to Login</Link>
                </p>
            </section>
        </main>
    );
