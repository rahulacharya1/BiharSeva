...
export function VolunteerVerifyOtpPage() {
  ...
  const tokenFromStorage = sessionStorage.getItem("otp_reset_token") || "";
  const otpResetToken = tokenFromState || tokenFromStorage;
  const email = useMemo(() => {
    return location.state?.email || sessionStorage.getItem("otp_reset_email") || "";
  }, [location.state]);
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await api.post("/volunteers/verify-otp/", {
        ...form,
        otp_reset_token: otpResetToken,
      });
      setMessage({ type: "success", text: res.data.message || "Password reset successful!" });
      sessionStorage.removeItem("otp_reset_token");
      sessionStorage.removeItem("otp_reset_email");

      // Short delay for user to see success message before redirect
    ...
    <main className="min-h-screen pb-24 bg-white">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6">
          ...
          {email ? (
            <p className="mt-8 max-w-2xl mx-auto text-lg text-slate-500 font-medium leading-relaxed">
              Humne ek 6-digit code <span className="text-slate-900 font-bold">{email}</span> par bheja hai. Use niche enter karein.
            </p>
          ) : (
            <p className="mt-8 max-w-2xl mx-auto text-lg text-slate-500 font-medium leading-relaxed">
              Apne mobile/email par bheje gaye OTP ko enter karke password update karein.
            </p>
          )}
        </div>
      </div>

      {/* VERIFY FORM (Overlap Card) */}
      <section className="max-w-xl mx-auto px-6 -mt-32 relative z-20">
        ...
        <p className="text-center mt-10 text-slate-400 text-[10px] font-black uppercase tracking-widest">
          Code didn't arrive? <Link to="/volunteer/request-otp" className="text-indigo-600 underline">Request New OTP</Link>
        </p>
      </section>
    </main>
  );
