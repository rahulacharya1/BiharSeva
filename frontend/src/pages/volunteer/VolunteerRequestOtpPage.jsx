...
export function VolunteerRequestOtpPage() {
    ...
    const submit = async (e) => {
        ...
        try {
            const res = await api.post("/volunteers/request-otp/", form);
            setMessage({ type: "success", text: res.data.message || "OTP has been sent to your device." });
            if (res.data.otp_reset_token) {
                sessionStorage.setItem("otp_reset_token", res.data.otp_reset_token);
                sessionStorage.setItem("otp_reset_email", form.email);
            }
            navigate("/volunteer/verify-otp", {
                state: {
                    otp_reset_token: res.data.otp_reset_token,
                    email: form.email,
                },
            });
        } catch (err) {
            ...
            <div className="relative bg-slate-50 border-b border-slate-200/60 pb-48 pt-20 overflow-hidden text-center">
                ...
                <div className="relative z-10 max-w-7xl mx-auto px-6">
                    ...
                    <h1 className="font-display text-5xl md:text-7xl font-bold tracking-tight text-slate-900 leading-[1.1]">
                        Request <span className="text-indigo-600">OTP.</span>
                    </h1>
                    <p className="mt-8 max-w-2xl mx-auto text-lg text-slate-500 font-medium leading-relaxed">
                        One-Time Password ke zariye apna account verify karein. BiharSeva security ensures aapka data hamesha protected rahe.
                    </p>
                </div>
            </div>

            {/* OTP FORM (Overlap Card) */}
            <section className="max-w-xl mx-auto px-6 -mt-32 relative z-20">
                ...
                >
                    <div className="bg-slate-900 px-10 py-4 flex justify-between items-center">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic text-center w-full">2-Step Verification</span>
                    </div>
                    ...
                    <form onSubmit={submit} className="p-10 md:p-14 space-y-6">
                        ...
                        <div className="pt-4 space-y-6">
                            ...
                            >
                                {loading ? <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : <>Send One-Time Password <FiSmartphone className="group-hover:rotate-12 transition-transform" /></>}
                            </button>
